...
- **Two Composed Layers**: `engine.py` normalises detector direction vocabularies onto a single bull/bear axis and **fuses** them into a tilt and conviction; `decision.py` runs on the same detections and adds redundancy discounting, conflict demotion and **position sizing** with an audit trail. `engine.scan_asset()` calls both and returns them together, so they cannot disagree about their inputs. `engine.py` also provides multi-ticker universe scanning, tiered caching, and programmatic verification that generated narration cites only real figures.
